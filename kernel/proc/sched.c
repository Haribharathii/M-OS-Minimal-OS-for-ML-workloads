...
{	
	...
	if(kthr->kt_state == KT_SLEEP_CANCELLABLE && kthr->kt_wchan){
		if(curproc->p_comm[0] == 'f'){ 
			//dbg(DBG_PRINT,"(GRADING1C 5)\n");
}
...
sched_switch(void)
{
	...
	//
      	intr_wait();
        intr_setipl(IPL_HIGH);/* protect runqueue again */
    } 
...
sched_make_runnable(kthread_t *thr)
{
	...
	// remove thread from whichever queue its blocked on	
	if(thr->kt_wchan) ktqueue_remove(thr->kt_wchan, thr);
	...
	thr->kt_state = KT_RUN;
	ktqueue_enqueue(&kt_runq, thr); //enqueue the thread
