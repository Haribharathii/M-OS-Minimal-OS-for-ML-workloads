...
int copy_to_user(void *uaddr, const void *kaddr, size_t nbytes)
{
        if (!range_perm(curproc, uaddr, nbytes, PROT_WRITE)) {
		 dbg(DBG_TEST, "copy_to_user: range_perm failed for uaddr=%p nbytes=%d\n", uaddr, nbytes);
                return -EFAULT;
        }
...
{
	KASSERT(NULL != p);
	KASSERT(p->p_vmmap != NULL);

	uint32_t vfn = ADDR_TO_PN((uintptr_t)vaddr);
...
range_perm(struct proc *p, const void *avaddr, size_t len, int perm)
{
	...
	uint32_t vfn  = ADDR_TO_PN(start);
	uint32_t stop = ADDR_TO_PN(end);
