...
vnode_init(void)
{
	...
	dbg(DBG_TEST, "vnode_mmobj_ops at %p\n", &vnode_mmobj_ops);
}
...
vref(vnode_t *vn)
{
        ...
        vn->vn_refcount++;
	if (vn->vn_vno == 84) {
             dbg(DBG_TEST, "Proc : %d vref: vnode 84 refcount up to %d\n", curproc->p_pid, vn->vn_refcount - 1);
        }
        dbg(DBG_VNREF, "%d vref: 0x%p, 0x%p ino %ld up to %d, nrespages=%d\n", curproc->p_pid,
...
vput(struct vnode *vn)
{
	...
	if (vn->vn_vno == 84) {
             dbg(DBG_TEST, "Proc : %d vput: vnode 84 refcount down to %d\n", curproc->p_pid, vn->vn_refcount - 1);
        }
        dbg(DBG_VNREF, "proc : %d vput: 0x%p, 0x%p ino %ld, down to %d, nrespages = %d\n",
            curproc->p_pid, vn, vn->vn_fs, (long)vn->vn_vno, vn->vn_refcount - 1, vn->vn_nrespages);
