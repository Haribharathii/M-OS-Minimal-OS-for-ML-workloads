...
pframe_shutdown()
{
 ...
 if (npinned != 0) {
                dbg(DBG_TEST, "\n==== pframe_shutdown: %d pinned pages remain ====\n", npinned);

                list_iterate_begin(&pinned_list, pf, pframe_t, pf_link) {
                        mmobj_t *o = pf->pf_obj;
                        dbg(DBG_TEST,
                            "PINNED PF: pf=%p addr=%p obj=%p pagenum=%u "
                            "ops=%p (ref=%d nres=%d)\n",
                            pf,
                            pf->pf_addr,
                            o,
                ...
                dbg(DBG_TEST, "==== end pinned page dump ====\n");
        }
...
pframe_migrate(pframe_t *pf, mmobj_t *dest)
{
    ...
    /* If dest already has a page for this pagenum, just throw this one away. */
    if (pframe_get_resident(dest, pf->pf_pagenum) != NULL) {
        /* Caller should not be using pf after this; and ideally it’s not pinned. */
       // KASSERT(!pframe_is_pinned(pf));
	// pframe_unpin(pf); //new 
        pframe_clean(pf);       /* if needed */
        pframe_free(pf);
        return;
    }

    if (pf->pf_obj == dest) return; /* nothing to do */

    mmobj_t *src = pf->pf_obj;

    /* Remove from old object’s lists */
    list_remove(&pf->pf_olink);
    src->mmo_nrespages--;
	src->mmo_ops->put(src); //new
...
pframe_get(struct mmobj *o, uint32_t pagenum, pframe_t **result)
{
	...
	pframe_t * newP = NULL;
	*result = NULL;
	if (NULL != (newP = pframe_get_resident(o, pagenum))) {
		KASSERT(!pframe_is_free(newP) && "residant page marked as free?!?!?!\n");
		while (pframe_is_busy(newP)) {
		    sched_sleep_on(&newP->pf_waitq);
		}
		*result = newP;
		return 0;
	...
	newP = pframe_alloc(o, pagenum);
	if(newP == NULL){
		return -ENOMEM;
	}
	int ret = pframe_fill(newP);
	if(ret < 0){
		pframe_free(newP);
		return ret;
	}
    ...
    *result = pframe_get_resident(o, pagenum);
	dbg(DBG_TEST, "Proc : %d here to get a page %d\n", curproc->p_pid, pagenum);
    if (*result == NULL){
